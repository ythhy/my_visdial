...
if __name__ == '__main__':
    ...
    f = h5py.File('img_train_resnet.h5',"w")
    img_info = h5py.File('img_train_info.h5',"r")
    train_img_features = f.create_dataset(
        'resnet_feat', (123287, 36, 2048), 'f')
    ...
    with open(infile, "r+b") as tsv_in_file:
        ...
        for item in reader:
            ...
            image_id = int(item['image_id'])
            id = np.argwhere(img_info==image_id)
            ...
            #     dtype=np.float32).reshape((item['num_boxes'], -1))
            train_img_features[id, :, :] = np.frombuffer(
                base64.decodestring(item['features']),
