...
def create_mats(data, ques_idx, ans_idx, cap_idx, params):
    ...
    round_len = np.zeros([N], dtype='uint32')
    ...
    return captions, caption_len, questions, question_len, answers, answer_len, \
           options, options_list, options_len, answer_index, image_list, round_len

def create_mats_test(data, ques_idx, ans_idx, cap_idx, params):
    N = len(data['data']['dialogs'])
    num_round = 10
    max_ques_len = params.max_ques_len
    max_cap_len = params.max_cap_len
    ...
    return captions, caption_len, questions, question_len, answers, answer_len, \
           options, options_list, options_len,  image_list, round_len
...
def img_info(image_list, split):
    ...
    for i, imgId in enumerate(image_list):
        jimg = {}
        jimg['imgId'] = imgId
        file_name = 'VisDialog_%s_%012d.jpg' % (split, imgId)
        path = 'VisualDialog_'+split
        jimg['path'] = os.path.join(path, file_name)
        out.append(jimg)
...
if __name__ == "__main__":
    ...
    # Input files
    parser.add_argument('-input_json_train', default='../data/visdial_1.0_train.json', help='Input `train` json file')
    parser.add_argument('-input_json_val', default='../data/visdial_1.0_val.json', help='Input `val` json file')
    parser.add_argument('-input_json_test', default='../data/visdial_1.0_test.json', help='Input `test` json file')

    # Output files
    parser.add_argument('-output_json', default='visdial_params_1.0.json', help='Output json file')
    parser.add_argument('-output_h5', default='new2_visdial_data_test_1.0.h5', help='Output hdf5 file')
    ...
    parser.add_argument('-word_count_threshold', default=5, type=int,
                        help='Min threshold of word count to include in vocabulary')
    ...
    ques_tok_test, ans_tok_test, cap_tok_test = tokenize_data(data_test)
    ...
    ques_idx_test, ans_idx_test, cap_idx_test = encode_vocab(ques_tok_test, ans_tok_test, cap_tok_test, wtoi)


    cap_train, cap_len_train, ques_train, ques_len_train, ans_train, ans_len_train, \
    opt_train, opt_list_train, opt_len_train, ans_index_train, image_list_train,round_len_train = \
    ...
    opt_val, opt_list_val, opt_len_val, ans_index_val, image_list_val,round_len_val = \
        create_mats(data_val, ques_idx_val, ans_idx_val, cap_idx_val, args)

    cap_test, cap_len_test, ques_test, ques_len_test, ans_test, ans_len_test, \
    opt_test, opt_list_test, opt_len_test,  image_list_test, round_len_test= \
        create_mats_test(data_test, ques_idx_test, ans_idx_test, cap_idx_test, args)
    ...
    # f.create_dataset('round_len_train', dtype='uint32', data=round_len_train)
    #
    ...
    # f.create_dataset('round_len_val', dtype='uint32', data=round_len_val)
    ...
    f.create_dataset('ans_len_test', dtype='uint32', data=ans_len_test)
    f.create_dataset('cap_test', dtype='uint32', data=cap_test)
    f.create_dataset('cap_len_test', dtype='uint32', data=cap_len_test)
    f.create_dataset('opt_test', dtype='uint32', data=opt_test)
    f.create_dataset('opt_len_test', dtype='uint32', data=opt_len_test)
    f.create_dataset('opt_list_test', dtype='uint32', data=opt_list_test)
    f.create_dataset('round_len_test', dtype='uint32', data=round_len_test)
    # f.close()
    ...
    # out['img_test'] = img_info(image_list_test, 'test2018')
    #
