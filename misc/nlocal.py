...
class NonLocalBlockFC(nn.Module):
    def __init__(self, in_channels, inter_channels=None,  bn_layer=True):
        super(NonLocalBlockFC, self).__init__()
        ...
        if bn_layer:
            ...
            self.W = nn.Linear(self.in_channels, self.inter_channels)
            nn.init.constant(self.W.weight, 0)
            nn.init.constant(self.W.bias, 0)
    ...
    def forward(self, x):
        ...
        g_x = self.g(x).view(batch_size, -1, self.inter_channels)
        # g_x = g_x.permute(0, 2, 1)

        theta_x = self.theta(x).view(batch_size, -1, self.inter_channels)
        # theta_x = theta_x.permute(0, 2, 1)
        phi_x = self.phi(x).view(batch_size, -1, self.inter_channels)
        ...
        W_y = nn.BatchNorm1d(self.in_channels).cuda()(W_y)
        W_y  =W_y.permute(0,2,1)
        z = W_y + x
